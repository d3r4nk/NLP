...
- Lay TOP N game theo so luong danh gia (Steam Search sort_by=Reviews_DESC)
...
Ghi chu:
- Steam review tieng Anh thuong nhieu hon. Neu game thieu review EN, script van chay va stats se phan anh.
- Steam search HTML co the thay doi, neu parse appid fail can cap nhat regex data-ds-appid.
"""
...
def fetch_search_html(timeout: int = 30) -> str:
    # sort_by=Reviews_DESC ~ sáº¯p theo sá»‘ lÆ°á»£ng user reviews giáº£m dáº§n
    params = {
...
def parse_top_appids(html: str, top_n: int) -> List[int]:
    # má»—i item thÆ°á»ng cĂ³ data-ds-appid="730" hoáº·c "730,123"
    out: List[int] = []
    seen = set()
...
def write_csv_from_jsonl(jsonl_path: str, csv_path: str) -> None:
    # Náº¿u khĂ´ng cĂ³ jsonl (vĂ­ dá»¥ game khĂ´ng cĂ³ review tiáº¿ng Viá»‡t), táº¡o CSV rá»—ng.
    ...
    if not rows:
        # táº¡o CSV rá»—ng vá»›i vĂ i cá»™t cÆ¡ báº£n
        fieldnames = ["source", "appid", "review_id", "created_at", "voted_up", "review_text"]
        os.makedirs(os.path.dirname(csv_path), exist_ok=True)
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=fieldnames)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for rank, appid in enumerate(appids, start=1):
        ...
        batch_stats["games"].append(game_entry)
